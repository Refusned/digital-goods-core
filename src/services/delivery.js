 ...
 * Срок аренды позиции.
 *
 * Покрывает обычный путь выдачи: несколько попыток к двум поставщикам с таймаутами и бэкоффом.
 * Отбраковки негодных кодов в расчёт НЕ закладываются намеренно: они редки, а их учёт
 * растягивал аренду вчетверо, и после аварийной остановки процесса позиция ждала снятия
 * протухшей аренды почти минуту вместо пятнадцати секунд.
 *
 * Истечение аренды посреди работы безопасно: повторный заход пойдёт к поставщику с тем же
 * request_id и получит тот же код, а две выдачи по одной позиции физически не вставятся
 * (deliveries.order_item_id это первичный ключ).
 */
const leaseMs = () => config.supplierTimeoutMs * config.supplierMaxAttempts * 2 + 5000;
