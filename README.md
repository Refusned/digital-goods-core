...
```bash
npm test                      # 33 теста: ядро, гонки, таймауты, восстановление, витрина
...
`npm test` работает сразу после `docker compose up -d`: он сам создаёт отдельную базу
`<имя>_test`, накатывает туда миграции и поднимает API с обеими заглушками поставщиков
на случайных портах. Рабочие данные тесты не трогают. Своя база задаётся через `TEST_DATABASE_URL`.
